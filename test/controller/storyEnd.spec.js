...
const providerId = 'storyStartEnd';
...
describe('storyEnd', function () {
    describe('auth fail', () => {
        it('sessionId', async () => {
            const params = {};

            const result = await testRequest(urlStoryEnd, params);
    ...
    describe('auth success', () => {
        ...
        it('start key required', async () => {
            ...
            const { error } = result.body;
            assert.equal(error.code, ModelError.Code.requiredField.code);
            assert.notEqual(error.additional.indexOf('[startKey]') === -1, true);
        })

        it('wrong story', async () => {
            const storyId = 'test';
            const startKey = '123';
            const params = { sessionId, storyId, startKey };
