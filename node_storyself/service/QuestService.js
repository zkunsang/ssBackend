...
const Schema = {
  ...
  ACCEPT_INSERT_LIST: {
    key: "acceptInsertList",
    required: true,
    type: ValidType.ARRAY,
  },
  ACCEPT_UPDATE_LIST: {
    key: "acceptUpdateList",
    required: true,
    type: ValidType.ARRAY,
  },
  ...
  QUEST_ACTION_LOG_LIST: {
    key: "questActionLogList",
    required: false,
    type: ValidType.Array,
  },
  QUEST_COMPLETE_LOG_LIST: {
    key: "questCompleteLogList",
    required: false,
    type: ValidType.ARRAY,
  },
};
...
class QuestService extends Service {
  ...
  async getQuestInfo(storyId) {
    const uid = this.getUID();

    const questInfo = await this.getUserQuestStoryDao().findOne({
      storyId,
      uid,
    });
  ...
  async acceptQuest(storyId, questId) {
    ...
    const userQuestStory = await this.getUserQuestStoryDao().findOne({
      storyId,
      uid,
    });
    ...
    if (userQuestStory == null) {
      ...
      this.pushAcceptInsertList(
        new UserQuestStory({ storyId, uid, questAccept })
      );
    } else {
    ...
    if (questAccepted) {
      this.pushAcceptLogList(
        new QuestAcceptLog({ uid, storyId, questId, logDate })
      );
    }
  }
  ...
  async checkUserAction(storyId, actionList) {
    ...
    this.pushActionLog(
      actionList.map(
        (item) => new ActionLog({ uid, storyId, ...item, logDate })
      )
    );
    ...
    for (const action of actionList) {
      if (!userNewActionMap[action.actionId]) {
        const _questList = QuestStoryCache.getQuestListByTargetId(
          storyId,
          action.actionId
        );
        if (!_questList) continue;
    ...
    const userStoryAction = userQuestStory
      ? userQuestStory.getStoryAction()
      : {};
    const userQuestAccept = userQuestStory
      ? userQuestStory.getQuestAccept()
      : {};
    ...
    for (const questId of questList) {
      ...
      for (const goal of goalList) {
        ...
        let userCount = userStoryAction[goal.actionId]
          ? userStoryAction[goal.actionId]
          : 0;
      ...
      if (isClear) {
        ...
        const acceptDate = userQuestAccept[questId]
          ? userQuestAccept[questId]
          : 0;

        completeLogList.push({
          uid,
          storyId,
          questId,
          completeType,
    ...
    if (userQuestStory) {
      ...
      await userQuestStoryDao.insertOne(
        new UserQuestStory({ storyId, uid, ...updateObject })
      );
    }

    if (completeLogList.length > 0) {
      this.pushCompleteLog(
        completeLogList.map((item) => new QuestCompleteLog({ ...item }))
      );
    }

    return {
      clearQuest,
      rewardList: putInvenRewardList,
      storyAction: userStoryAction,
    };
  }

  async completeCommonQuest({ storyId, questIds }) {
    const uid = this.getUID();
    const logDate = this.getUpdateDate();
    const userQuestStoryDao = this.getUserQuestStoryDao();
