...
class InventoryService extends Service {
  constructor(userInfo, updateDate) {
    ...
    this[Schema.INVENTORY.key] = inventory
      ? ArrayUtil.map(inventory, (item) => new Inventory(item))
      : [];
    this[Schema.INVENTORY_MAP.key] = ArrayUtil.keyBy(
      this[Schema.INVENTORY.key],
      "itemId"
    );
  ...
  checkStoryMerge() {
    ...
    for (let i = 0; i < inventoryList.length; i++) {
      ...
      if (!itemData) {
        console.log(`${inventory.itemId} is null`);
        continue;
      }
  ...
  makeInventoryStoryList(storyIdList) {
    return ArrayUtil.map(
      storyIdList,
      (storyId) => new Inventory({ itemId: storyId, itemQny: 1 })
    );
  }
  ...
  throwAlreadyHasItem(itemId) {
    const uid = this.getUID();
    throw new SSError.Service(
      SSError.Service.Code.alreadyHaveItem,
      `[${uid}]: itemId(${itemId})`
    );
  }
}
